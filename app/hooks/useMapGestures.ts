...
export function useMapGestures(onTap?: (screenX: number, screenY: number) => void) {
  const fitScale = Math.max(SCREEN_WIDTH / MAP_SIZE, MAP_AREA_HEIGHT / MAP_SIZE);
  ...
  const pinchGesture = Gesture.Pinch()
    ...
    .onUpdate((e) => {
      ...
      const originX = (initialFocalX.value - savedTranslateX.value) / savedScale.value;
      const originY = (initialFocalY.value - savedTranslateY.value) / savedScale.value;

      scale.value = newScale;
      translateX.value = savedTranslateX.value - originX * (newScale - savedScale.value) + deltaX;
      translateY.value = savedTranslateY.value - originY * (newScale - savedScale.value) + deltaY;
    })
    .onEnd(() => {
      savedScale.value = scale.value;
      savedTranslateX.value = translateX.value;
  ...
  const recenter = useCallback(
    (bounds: { minX: number; minY: number; maxX: number; maxY: number } | null) => {
      ...
      const newScale = Math.min(SCREEN_WIDTH / contentW, MAP_AREA_HEIGHT / contentH, 1.5);
