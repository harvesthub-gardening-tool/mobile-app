...
const GRASS_AREA = MAP_SIZE - 80; // inner area excluding border (40px each side)
...
function generateGrassDecorations() {
    ...
    let idx = 0;
    for (let y = 0; y < GRASS_AREA; y += GRASS_STEP) {
        for (let x = 0; x < GRASS_AREA; x += GRASS_STEP) {
            const seed = idx * 7919;
